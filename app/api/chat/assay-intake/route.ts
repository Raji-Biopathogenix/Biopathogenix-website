...
export async function POST(req: Request) {
  try {
    ...
    const backendEndpoint =
      process.env.ASSAY_INTAKE_BACKEND_URL ??
      `${(process.env.NEXT_PUBLIC_API_BASE_URL ?? DEFAULT_BACKEND_BASE).replace(/\/+$/, "")}/assay-intake/`;
    ...
    if (!upstream) {
      return NextResponse.json({ error: "Assay intake backend endpoint is not configured." }, { status: 500 });
    }
    ...
    if (!upstream.ok || looksLikeHtml(text)) {
      ...
      const error =
        upstream.status === 404
          ? "Assay intake backend endpoint not found"
          : looksLikeHtml(text)
    ...
    return NextResponse.json(
      { error: "Backend not connected", detail },
      { status: 500 }
    );
  }
}
