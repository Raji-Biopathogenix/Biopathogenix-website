...
export async function POST(req: Request) {
  try {
    ...
    const backendEndpoint =
      ...
      `${(process.env.NEXT_PUBLIC_API_BASE_URL ?? DEFAULT_BACKEND_BASE).replace(/\/+$/, "")}/chat/`;
    ...
    for (const url of candidates) {
      ...
      // A non-404 response is definitive; stop retrying.
      if (upstream.status !== 404) break;
    }

    if (!upstream) {
      return NextResponse.json({ error: "Chat backend endpoint is not configured." }, { status: 500 });
    }
    ...
    if (!upstream.ok || looksLikeHtml(text)) {
      ...
      const error =
        upstream.status === 404
          ? "Chat backend endpoint not found"
          : looksLikeHtml(text)
