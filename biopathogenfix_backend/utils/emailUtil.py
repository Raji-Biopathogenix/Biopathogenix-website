...
class CustomLogger:
    def __init__(self, app_name, filename=None):
        ...
        self.logger.setLevel(getattr(logging, configSettings.LOG_LEVEL.upper(), logging.INFO))
    ...
    def send_critical_email(self, message):
        recipient = configSettings.ERROR_RECIPIENT or getattr(settings, 'ADMINS', None)
        if not recipient:
        ...
        try:
            ...
            if getattr(settings, "GRAPH_ENABLED", False):
                send_graph_email(emails, subject, text_body=body, from_email=configSettings.DEFAULT_FROM_EMAIL)
            else:
                send_mail(subject, body, configSettings.DEFAULT_FROM_EMAIL, emails, fail_silently=False)
...
class EmailService:
    ...
    def send_plain(to_list, subject, message, from_email=None):
        from_email = from_email or configSettings.DEFAULT_FROM_EMAIL
        if getattr(settings, "GRAPH_ENABLED", False):
            send_graph_email(to_list, subject, text_body=message, from_email=from_email)
        else:
...
def generate_otp(length=None):
    length = length or configSettings.OTP_LENGTH
    start = 10 ** (length - 1)
    end = (10 ** length) - 1
...
def send_otp(email, template='otp_email.html', context_extra=None):
    """
    Rate-limited OTP sender. Stores OTP in cache with TTL configSettings.OTP_TTL_SECONDS.
    Returns (ok: bool, msg: str)
    """
    email = email.lower()
    rate_key = _otp_rate_key(email)
    counter = cache.get(rate_key) or 0
    if counter >= configSettings.OTP_RATE_LIMIT_MAX:
        return False, "Too many OTP requests. Try again later."

    otp = generate_otp()
    otp_key = f"otp:{email}"
    cache.set(otp_key, otp, configSettings.OTP_TTL_SECONDS)
    cache.incr(rate_key) if cache.get(rate_key) is not None else cache.set(rate_key, 1, configSettings.OTP_RATE_LIMIT_WINDOW)
